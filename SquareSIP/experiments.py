...
radius = .1                             # Square obstacle radius (1/2 * side length)
...
def linearized_sip_constraint(x_k, x_k1, x_new_k, x_new_k1, c, radius):
    ...
    # 1. Gather your analytical candidates for t
    candidates = [0.0, 1.0]  # Endpoints
    
    # Avoid zero divisions for individual coordinate crossings
    if np.abs(b[0]) > 1e-8:
        candidates.append(-a[0] / b[0])
    if np.abs(b[1]) > 1e-8:
        candidates.append(-a[1] / b[1])
        
    # Candidates of t are when |a1 + b1*t| == |a2 + b2*t|
    if np.abs(b[0] - b[1]) > 1e-8:
        candidates.append((a[1] - a[0]) / (b[0] - b[1]))
    if np.abs(b[0] + b[1]) > 1e-8:
        candidates.append((-a[0] - a[1]) / (b[0] + b[1]))

    # Going thru all the candidates to determine which is t_star
    t_star = 0.0
    ...
    for t in candidates:
        if 0.0 <= t <= 1.0:
            ...
            if dist_test < min_distance:
                min_distance = dist_test
                t_star = t

    # 3. Extract coordinates at the worst-case location
    closest_to_center = x_k + t_star * b
    diff = closest_to_center - c
    
    if min_distance < 1e-6:
        min_distance = 1e-6

    # 4. Compute the Infinity-Norm Gradient (First-Order Derivative)
    # For a square, the push direction is purely along the dominant face axis
    offset = radius - min_distance
    gradient = np.zeros(2)
    
    if np.abs(diff[0]) >= np.abs(diff[1]):
        gradient[0] = -np.sign(diff[0])  # Push purely along the X-axis
    else:
        gradient[1] = -np.sign(diff[1])  # Push purely along the Y-axis

    # 5. Distribute using the Chain Rule to your new variables
    return offset + (1 - t_star) * gradient @ (x_new_k - x_k) \
                  + t_star * gradient @ (x_new_k1 - x_k1) <= 0
